...
let test_apply_action_bet_calls _ =
  let round1 = get_from_result (Round.apply_action round player_tobi (Bet (Chips.of_int 10))) in
  assert_equal (Chips.of_int 10) @@ Round.get_contribution round1 player_tobi;
  let round2 = get_from_result (Round.apply_action round1 player_vrinda Call) in
  assert_equal (Chips.of_int 10) @@ Round.get_contribution round2 player_vrinda;
  let round3 = get_from_result (Round.apply_action round2 player_timmy Call) in
  assert_equal (Chips.of_int 10) @@ Round.get_contribution round3 player_timmy;
  assert_equal true @@ Round.is_over round3

let test_apply_action_raise _ =
  let round1 = get_from_result (Round.apply_action round player_tobi (Bet (Chips.of_int 10))) in
  let round2 = get_from_result (Round.apply_action round1 player_vrinda (Raise (Chips.of_int 15))) in
  assert_equal (Chips.of_int 25) @@ Round.get_contribution round2 player_vrinda;
  let round3 = get_from_result (Round.apply_action round2 player_timmy Call) in
  assert_equal (Chips.of_int 25) @@ Round.get_contribution round3 player_timmy;
  assert_equal false @@ Round.is_over round3;
  let round4 = get_from_result (Round.apply_action round3 player_tobi Call) in
  assert_equal (Chips.of_int 25) @@ Round.get_contribution round4 player_tobi
