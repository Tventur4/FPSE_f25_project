...
let make_move (bot : t) (stage : Card.betting_round) (community_cards : Card.t list) (num_players : int) (hole_cards : Card.t * Card.t) (chips : int) : Card.action =
  ...
  | Always_fold -> Fold
  | All_in -> Bet chips
  | Rule_hand_only -> Bot_hand_only.rule_hand_only_move stage community_cards hole_cards chips (*cdecides move based off of its hand only *)
  | Rule_best_hand -> Bot_best_hand.rule_best_hand_move stage community_cards num_players hole_cards chips (* unimplemented, decides move based off of the probability it has the best hand *)
